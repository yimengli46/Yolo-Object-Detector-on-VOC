...
class VocDetectorDataset(DataLoader.Dataset):
    ...
    def encoder(self, boxes, labels):
        ...
        wh = boxes[:, 2:] - boxes[:, :2]    #num_boxes x 2
        center_xy_all = (boxes[:, 2:] + boxes[:, :2]) / 2   # coords of the center of the bbox
        for i in range(center_xy_all.size()[0]):
            center_xy = center_xy_all[i] # (center_x, center_y)
            ij = (center_xy / cell_size).ceil() - 1 # (i_cell, j_cell)
            ...
            xy = ij * cell_size  # coordinates of the center
